...
# 1. 页面配置
st.set_page_config(page_title="财务全图谱-V69.7", layout="wide")
...
def st_plotly_line(x, y, name, unit="", color=None):
    ...
    fig.update_layout(height=300, margin=dict(l=10, r=10, t=30, b=10), xaxis_type='category')
...
def run_v69_engine(ticker, is_annual):
    try:
        ...
        # --- 1. 顶部：公司概况 ---
        st.title(f"🏛️ 财务审计图谱 V69.7：{info.get('longName', ticker)}")
        with st.expander("🏢 查看公司主营业务与商业模式", expanded=True):
            c_info1, c_info2 = st.columns([1, 2])
            with c_info1:
                st.write(f"**行业**：{info.get('industry', '未知')}"); st.write(f"**板块**：{info.get('sector', '未知')}")
            with c_info2:
                st.write("**业务摘要**："); st.write(info.get('longBusinessSummary', '暂无描述')[:500] + "...")
        ...
        with col_d:
            st.subheader("📝 核心诊断")
            st.write(f"ROE: {roe.iloc[-1]:.2f}% | 现金流/利润: {(ocf/ni).iloc[-1]:.2f} | 负债率: {debt_ratio.iloc[-1]:.1f}%")
        ...
        with rc2: st_plotly_line(years, (rev/assets).fillna(0), "资产周转率", "次", "#0083B8")
        with rc3: st_plotly_line(years, (assets/equity).fillna(0), "权益乘数", "倍", "#2E7D32")
        ...
        with c31: 
            ...
            st_plotly_bar(years, c2c, "C2C周期 (天)", "#7D3C98")
        with c32:
        ...
        with sc3: st_plotly_line(years, int_cover, "利息保障倍数", "次", "#27AE60")
